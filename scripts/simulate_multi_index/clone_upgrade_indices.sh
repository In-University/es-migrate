...
# Step 1: Read all aliases and extract concrete indices
...
# Fallback: if no aliases found, fetch non-system indices
...
# Step 2: Clone each concrete index to <index>_upgrade
for SRC_INDEX in $CONCRETE_INDICES; do
  DST_INDEX="${SRC_INDEX}${SUFFIX}"
  echo ""
  echo ">> Cloning Index: '$SRC_INDEX' -> '$DST_INDEX'"

  # Check if target index already exists (SAFETY: DELETE ONLY TARGET DST_INDEX, NEVER SRC_INDEX)
  ...
  # 1. Create target index with high-speed indexing settings
  ...
  # 2. Run Parallel Sliced Reindex
  echo "   Reindexing via parallel slices & batch size $BATCH_SIZE..."
